...
Each request is bound to the exact authorization grant captured when streaming starts. A later enable cannot be adopted, and reset or re-enable invalidates the captured grant. User images and computer screenshots preceding a later completed assistant message are replaced with bounded historical-image placeholders instead of being routed again; reasoning or function_call items alone do not count, because vision descriptions are request-ephemeral and must survive mid tool-loop turns. This consumed-history rewrite runs both before and after the caller payload hook.
