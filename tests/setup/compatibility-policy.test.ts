...
const range = ">=0.80.1 <0.85.0 || >=0.85.1 <0.88.0";
const fixture = (): RangePolicy => ({
  peerRange: range,
  minimum: "0.80.1",
  latest: "0.87.1",
  unsupported: { older: "0.79.10", excluded: ["0.85.0"], upper: "0.88.0" },
});
...
describe("bounded Pi compatibility policy", () => {
  ...
  it.each(["0.80.1", "0.80.8", "0.84.4", "0.85.1", "0.85.9", "0.86.0", "0.86.1", "0.87.0", "0.87.1"])("accepts supported release %s", (version) => {
    expect(satisfiesPeerRange(version, range)).toBe(true);
  });

  it.each(["0.79.10", "0.80.0", "0.85.0", "0.88.0", "1.0.0"])("rejects unsupported release %s", (version) => {
  ...
  it("rejects widening through a known-broken release", () => {
    expect(() => verifyRangePolicy({ ...fixture(), peerRange: ">=0.80.1 <0.88.0" }))
  ...
  it.each(["0.80.1", "0.84.4", "0.85.0", "0.88.0"])("rejects latest endpoint %s outside the final interval", (latest) => {
  ...
  it("rejects inconsistent lower and upper endpoints", () => {
    ...
    const invalid = fixture();
    invalid.unsupported.upper = "0.89.0";
  ...
  it.each(["0.79.10", "0.88.0", "0.85.1"])("rejects misplaced exclusion %s", (excluded) => {
    const invalid = fixture();
