...
- a proxy HTTP 400 containing the narrow `encrypted_content` marker, or a status-less HTTP-200 SSE `response.failed` carrying both `invalid_request` and that marker, is classified with fixed, redacted clean-session/model guidance;
- an `xai-auth` stream retries that mismatch once within the same turn only when the fully validated outbound payload contained encrypted reasoning, no assistant content was forwarded, and the combined transport signal is not aborted. The rejected attempt's start/error events are withheld; its retry strips replay input after caller hooks, keeps the encrypted-output include policy and visible/tool history, repeats local guards, retains redirect protection and session affinity, and generates a new request ID;
- an unsuccessful retry returns the same fixed guidance without a third attempt; cancellation and local pre-send validation errors retain their own behavior. No replayed reasoning, already-forwarded content, or unrelated failure means no automatic retry. Direct Responses helpers remain non-retrying;
- after that fixed failure is retained in same-model history, the next request omits encrypted reasoning items from the rejected chain while preserving visible messages, function calls, tool results, and the `store:false` / encrypted-include policy; unrelated failures do not activate this omission. A successful turn resumes normal eligible replay.
...
Pi's delegated SSE error shape may not preserve an HTTP status for failures that arrive after a successful stream connection. When a status is available (including one extracted from delegate text), the mismatch classifier requires status 400 plus the `encrypted_content` marker. Without a status, classification is separately bounded to a terminal `response.failed` result whose delegate text contains both xAI's `invalid_request` code and that marker. The retry gate compares only the resulting fixed, redacted message. Other streamed failures remain generic and redacted, and numeric status/retry wording is preserved when the delegate supplies it.
